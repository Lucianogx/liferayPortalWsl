...
declare type Variables = Record<string, any> | null;
declare type LinkFunction = (variables: string) => Promise<any>;
declare type FetchRetryDelay = {
    /**
...
declare type Props = {
     ...
     */
    fetch?: <T = unknown>(link: string, init?: RequestInit | undefined) => Promise<Response> | Promise<FetchCursor<T>> | Promise<T>;
    /**
...
declare const useResource: ({ fetch: fetcher, fetchDelay, fetchOptions, fetchPolicy, fetchRetry, fetchTimeout, link, onNetworkStatusChange, pollInterval, storageMaxSize, suspense, variables, }: Props) => {
    loadMore: () => Promise<any> | null;
    refetch: () => void;
    resource: any;
};
declare const Resource: (props: Props) => JSX.Element;
